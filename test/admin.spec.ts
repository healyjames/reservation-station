...
describe('DELETE /api/admin/reservations/:id', () => {
  ...
  it('returns 404 when booking belongs to another tenant - resource existence is not revealed', async () => {
    ...
    const row = await env.maximum_bookings_db.prepare('SELECT id FROM Reservations WHERE id = ?').bind(RES_ID_OTHER).first();
...
describe('PATCH /api/admin/me', () => {
  ...
  it('ignores id and tenant_id fields in the request body - they are immutable', async () => {
    ...
    const original = await env.maximum_bookings_db.prepare('SELECT id FROM Tenants WHERE id = ?').bind(TENANT_ID).first();
    expect(original).not.toBeNull();

    const spoofed = await env.maximum_bookings_db.prepare('SELECT id FROM Tenants WHERE id = ?').bind(spoofedId).first();
