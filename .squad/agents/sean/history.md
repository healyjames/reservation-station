...
- **Rolling occupancy capacity (2026-06-12)**: `max_covers` is the venue-wide concurrent seat cap, not a day total. `GET /api/reservations/blocked-times` must read `max_covers` directly, and `POST /api/reservations` must load same-day `reservation_time`/`guests` rows and call `calculateConcurrentGuests()` with `concurrent_guests_time_limit` before rejecting with `Insufficient capacity for the requested time`. The legacy `/daily-capacity` route and related docs/tests/types were removed because the product no longer exposes a day-level covers model.

- **C-3 tenant super-admin protection (2026-06-12)**: Tenant list/create/update/delete now use a dedicated `superAdminAuth` middleware that checks `X-Admin-Key` against `env.SUPER_ADMIN_KEY`, mirroring the simple header-guard pattern from `adminAuth` but preserving the tenants router's existing `{ error }` response style. `GET /api/tenants/:id` stays public for widget consumption, but must project only widget-safe tenant fields and exclude `contact_email`, `created_date`, and `modified_date` before returning the row.
...
- **contact_email required (2026-06-01)**: Made `contact_email` NOT NULL on Tenants. Migration updated in-place to `NOT NULL DEFAULT ''` (SQLite requires a DEFAULT for NOT NULL ADD COLUMN against existing rows). Schema changed from `.nullable().optional()` to `.string().email()`. All three `waitUntil` null-guards in `reservations.ts` (POST, PATCH, DELETE) removed. Type annotations updated from `string | null` to `string`.

 on POST/PATCH/DELETE reservation routes. `sendEmail()` POSTs to Resend API, throws on non-2xx. Six template files (`src/emails/*.ts`) are self-contained — each has its own `detailsTable()` and `emailWrapper()` helpers (leemunroe inline-style pattern, brand `#266663`). All routes use `c.executionCtx.waitUntil((async () => { ... })())` IIFE pattern — HTTP response never blocked by email delivery. `contact_email` null-guard: skip all emails + `console.warn`. PATCH fetches joined row pre-update (404 check) and re-fetches post-update (amendment emails use persisted state). DELETE fetches joined row pre-delete (for email context). Migration `0006_tenants_contact_email.sql` adds `contact_email TEXT` to Tenants. `RESEND_API_KEY` added to `Env` interface in `env.d.ts`.
