...
- **BlockedDates system (migration 0004)**: Replaced `block_current_day` boolean on Tenants with a proper `BlockedDates` table (id, tenant_id, date, start_time, end_time, reason). Full-day blocks have `start_time IS NULL`; partial blocks have explicit `[start_time, end_time)` ranges. Migration drops the old column and creates the new table with a compound `(tenant_id, date)` index. Route mounting: admin CRUD at `/api/admin/blocked-dates` (own router, own file, `adminAuth` via `blockedDates.use('*', adminAuth)`). Public calendar endpoint at `GET /api/reservations/blocked-dates?tenant_id=&month=YYYY-MM` returns only full-day blocked dates. `GET /blocked-times` now checks BlockedDates before capacity: full-day block → all slots; partial blocks bypass capacity logic and are merged with capacity-blocked slots. `POST /reservations` checks BlockedDates for full-day block before inserting.
